...
const generateRecipe = async (req, res) => {
  try {
    ...
    const actMultiplier = activityMultipliers[userProfile.activityLevel] || 1.2;
    
    const proteinMultipliers = { 'sedentary': 0.8, 'light': 1.0, 'moderate': 1.2, 'active': 1.6 };
    const proteinFactor = proteinMultipliers[userProfile.activityLevel] || 0.8;

    const dailyCalories = Math.round(bmr * actMultiplier);
    const dailyProtein = Math.round(userProfile.weight * proteinFactor);

    const targetCalories = Math.round(dailyCalories * 0.35); 
    const targetProtein = Math.round(dailyProtein * 0.35);

    console.log(`📊 AI Profile: ${userProfile.activityLevel.toUpperCase()} | Allergies: ${userProfile.allergies.join(', ') || 'None'}`);
    console.log(`🎯 Targets: ${targetCalories} kcal | ${targetProtein}g Protein`);
    ...
    if (!response.data.results || response.data.results.length === 0) {
      return res.status(404).json({ error: "No safe recipes found matching your criteria." });
    }
