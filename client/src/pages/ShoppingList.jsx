...
import { Package, Trash2, Check, Calendar, ShoppingCart, LogOut, Loader } from 'lucide-react';
...
const convertToMetric = (amount, originalUnit) => {
  ...
  // Fallback for "pinch", "slice", etc. -> default to 'pcs'
...
const ShoppingList = () => {
  ...
  // --- 2. UPDATED HANDLE BOUGHT (Uses Converter) ---
  const handleBoughtClick = (listId, item) => {
    setSelectedListId(listId);
    setSelectedItem(item);

    // Auto-convert Spoonacular units to Metric
    const { q, u } = convertToMetric(item.amount || 1, item.unit || '');

    setBuyDetails({
      quantity: q, 
      unit: u,  // Now sets 'g', 'ml', 'pcs', etc.
      expiryDate: '', 
      category: 'Other'
    });
    setModalOpen(true);
  ...
  const confirmPurchase = async (e) => {
    ...
    try {
      ...
      await api.post('/inventory', {
        ...
        unit: buyDetails.unit, // Uses the dropdown value
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><Loader className="animate-spin text-green-600"/></div>;
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      <nav className="bg-white/80 backdrop-blur-md sticky top-0 z-40 shadow-sm border-b border-green-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
             <button onClick={() => navigate('/dashboard')} className="text-gray-500 hover:text-green-600 font-bold text-sm">Dashboard</button>
             <button onClick={handleLogout} className="flex items-center gap-2 text-gray-500 hover:text-red-600 text-sm font-semibold"><LogOut className="w-4 h-4" /> Sign Out</button>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-8">
          <div className="bg-green-100 p-3 rounded-full text-green-600"><ShoppingCart className="w-8 h-8"/></div>
          <div>
        ...
        {lists.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-2xl shadow border border-gray-100">
            ...
            <button onClick={() => navigate('/dashboard')} className="mt-4 text-green-600 font-bold hover:underline">Go to Dashboard</button>
          </div>
        ) : (
            ...
            {lists.map(list => (
              <div key={list._id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-50">
                  {list.items.length === 0 && <div className="p-6 text-center text-gray-400 text-sm">All items bought! Great job.</div>}
                  
                  {list.items.map(item => (
                    <div key={item._id} className="p-4 flex items-center justify-between hover:bg-green-50/30 transition-colors group">
                      <div className="flex items-center gap-4">
                        {item.image && <img src={item.image} alt={item.name} className="w-10 h-10 rounded-lg object-cover bg-gray-100"/>}
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleBoughtClick(list._id, item)} className="flex items-center gap-2 bg-green-100 text-green-700 px-4 py-2 rounded-lg font-bold text-sm hover:bg-green-200 transition-all shadow-sm">
                          <Check className="w-4 h-4" /> Bought
                        </button>
                        <button onClick={() => handleDeleteItem(list._id, item._id)} className="p-2 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
      ...
      {/* --- 3. MODAL WITH UNIT DROPDOWN --- */}
      {modalOpen && selectedItem && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            <h3 className="text-xl font-bold text-gray-800 mb-2">Add to Pantry</h3>
            <p className="text-sm text-gray-500 mb-6">Confirm details for <span className="font-bold text-gray-800 capitalize">{selectedItem.name}</span></p>

            <form onSubmit={confirmPurchase} className="space-y-4">
              
              {/* Quantity & Unit Row */}
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Quantity Bought</label>
                <div className="flex gap-2">
                  ...
                  {/* 👇 THE UNIT DROPDOWN YOU ASKED FOR 👇 */}
                  <select 
                    className="bg-gray-100 px-4 py-3 rounded-xl border border-gray-200 text-gray-600 font-bold focus:outline-none cursor-pointer"
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1 flex items-center gap-1">
                  <Calendar className="w-3 h-3"/> Expiry Date (Required)
                </label>
                <input 
                  type="date" required
                  className="w-full p-3 bg-white border-2 border-green-100 focus:border-green-500 rounded-xl outline-none transition-colors"
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setModalOpen(false)} className="flex-1 py-3 rounded-xl font-bold text-gray-500 hover:bg-gray-100">Cancel</button>
                <button type="submit" className="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold shadow-lg shadow-green-200">Confirm & Move</button>
              </div>
            </form>
          </div>
        </div>
