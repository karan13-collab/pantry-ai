...
import { Plus, Trash2, AlertCircle, LogOut, Package, Search, ChefHat, Leaf, Heart, ShoppingCart, X, Copy, Check, User } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-100 font-sans text-gray-800 relative">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all scale-100">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-800">Choose Chef's Goal 👨‍🍳</h3>
              <button onClick={() => setShowStrategyModal(false)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5"/></button>
            </div>
      ...
      <nav className="bg-white/80 backdrop-blur-md sticky top-0 z-40 shadow-sm border-b border-green-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab('pantry')}>
          ...
          <div className="animate-fade-in">
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-white p-6 mb-8">
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <div className="bg-green-100 p-1.5 rounded-md"><Plus className="w-5 h-5 text-green-700" /></div> Quick Add
              </h3>
              <form onSubmit={handleAddItem} className="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div className="md:col-span-4"><input type="text" placeholder="Item Name" required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl" value={newItem.name} onChange={(e) => setNewItem({...newItem, name: e.target.value})} /></div>
                <div className="md:col-span-2 flex"><input type="number" min="1" className="w-2/3 p-3 bg-gray-50 border border-gray-200" value={newItem.quantity} onChange={(e) => setNewItem({...newItem, quantity: e.target.value})} /><select className="w-1/3 bg-gray-100 border border-gray-200" value={newItem.unit} onChange={(e) => setNewItem({...newItem, unit: e.target.value})}><option>pcs</option><option>kg</option><option>L</option></select></div>
                <div className="md:col-span-3"><select className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl" value={newItem.category} onChange={(e) => setNewItem({...newItem, category: e.target.value})}><option>Other</option><option>Vegetable</option><option>Fruit</option><option>Dairy</option><option>Grain</option><option>Meat</option></select></div>
                <div className="md:col-span-3"><input type="date" required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl" value={newItem.expiryDate} onChange={(e) => setNewItem({...newItem, expiryDate: e.target.value})} /></div>
                <div className="md:col-span-12 flex justify-end"><button className="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2"><Plus className="w-5 h-5" /> Add</button></div>
              </form>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-8">
              <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-orange-50/50 to-transparent flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2"><ChefHat className="text-orange-500" /> AI Chef</h3>
                  <p className="text-sm text-gray-500">Intelligent meal planning.</p>
                </div>
                <button onClick={handleOpenChef} disabled={items.length === 0} className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2 rounded-full font-bold shadow-md flex items-center gap-2 disabled:opacity-50 transition-all">
                  Suggest Recipe
                </button>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr><th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Item</th><th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Category</th><th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Qty</th><th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Status</th><th className="px-6 py-4"></th></tr>
                  </thead>
                    ...
                    {filteredItems.map((item) => {
                      ...
                      return (
                        <tr key={item._id} className="hover:bg-green-50/30">
                          ...
                          <td className="px-6 py-4"><span className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-full border">{item.category}</span></td>
                          <td className="px-6 py-4 text-sm">{item.quantity} {item.unit}</td>
                          <td className="px-6 py-4"><span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold border ${status.color}`}>{status.text === 'Expired' && <AlertCircle className="w-3 h-3" />} {status.text}</span></td>
                          <td className="px-6 py-4 text-right"><button onClick={() => handleDeleteItem(item._id)} className="text-gray-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button></td>
                        </tr>
                      );
                    })}
                  </tbody>
